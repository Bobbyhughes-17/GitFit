    ...
    public class WorkoutSplitRepository : BaseRepository, IWorkoutSplitRepository
    {
        ...
        public List<WorkoutSplit> GetAllWorkoutSplits()
        {
            ...
            using (var conn = Connection)
            {
                ...
                {
                    cmd.CommandText = @"
                        SELECT Id, SplitName, DaysPerWeek
                        FROM Workout_Splits";
        ...
        public WorkoutSplit GetWorkoutSplitById(int id)
        {
            ...
            using (var conn = Connection)
            {
                ...
                {
                    cmd.CommandText = @"
                SELECT ws.Id AS SplitId, ws.SplitName, ws.DaysPerWeek, wsd.Id, wsd.ExerciseID, wsd.SplitID, wsd.DayOfWeek,
        ...
        public void AddWorkoutSplit(WorkoutSplit workoutSplit)
        {
            using (var conn = Connection)
            {
                ...
                {
                    cmd.CommandText = @"
                INSERT INTO Workout_Splits (SplitName, DaysPerWeek)
                OUTPUT INSERTED.ID
                VALUES (@SplitName, @DaysPerWeek)";
